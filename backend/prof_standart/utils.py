...
class FileUtil:
    resources_dir: str = os.path.join(os.path.dirname(backend.__file__), 'resources')
    ...
    def _get_recent_file(cls, dir_path) -> str:
        files = glob.glob(f'{dir_path}{os.path.sep}*')
    ...
    def read_recent_and_remove(cls, dir_path: str) -> str:
        ...
        try:
            with open(recent_file, encoding="utf8") as file:
                data: str = file.read()
